...
def hillClimbing (epsilon, N, a, b, funcion):
  ...
  while ((funcion(x_current) > epsilon) and (k < 100000)):
    i = 0
    y = []
    while i < N:
      y.append(x_current[i]+ np.random.uniform(-1,1))
      i = i + 1
    x.append(y)    
    #x.append([x_current[0]+ np.random.uniform(-1,1), x_current[1]+ np.random.uniform(-1,1)])

    if funcion(x[k+1]) < funcion(x_current):
      x_current = x[k+1]
    k=k+1
