...
from algoritmos import HMetodoSecante as sec
from algoritmos import BOUNDINGPhase as BP

st.title("Método de Secante")
st.markdown('<div style="text-align: justify;">En este método, se considera tanto la magnitud como el signo de la derivada para crear un punto nuevo. ' \
'Esto es debido a que se asume que la derivada varía linealmente entre los límites, ya que estas tienen derivadas con signos opuestos, ' \
'y por lo mismo hay un punto entre las dos que tiene la derivada cero. Para ello se usa la ecuación: </div>', unsafe_allow_html=True)

latext1 = r'''
$$ 
z = x_{2} - \frac{f'(x_{2})}{((f''(x_{2}) - f'(x_{1})) / (x_{2} - x_{1}))} 
$$  
'''
st.write(latext1)
...
st.latex(r"""
\begin{array}{l}
\textbf{Algoritmo: Método de Secante} \\
\rule{14cm}{1pt}\\
\textbf{Definir: } a, b \text{, de forma que } f'(a) \lt  0 \text{ y } f'(b) \gt  0, \epsilon, x_{1} = a, x_{2} = b.\\[0.5 ex] 
         \\
\textbf{Paso 1: } \text{Calcular z, considerando la ecuación mostrada previamente.} \\[0.5 ex] 
...
with col1:
    with st.form("my_form"):
        ...
        if(option == "Lata"):
            ...
            x1, px, py = sec.secante(a, b, values, funcionUnimodal.lata)
            ...
            x1, px, py = sec.secante(a, b, values, funcionUnimodal.caja)
            ...
            x1, px, py = sec.secante(a, b, values, funcionUnimodal.ej1)
            ...
            x1, px, py = sec.secante(a, b, values, funcionUnimodal.ej4)
            ...
            x1, px, py = sec.secante(a, b, values, funcionUnimodal.ej5)
            ...
            x1, px, py = sec.secante(a, b, values, funcionUnimodal.ej6)
            ...
            x1, px, py = sec.secante(a, b, values, funcionUnimodal.ej7)                   
